...
function dev_tools_from_source() {
    ...
    function install_python_from_source() {
        ...
        update-alternatives --install /usr/local/bin/python2 python2 "${py2}" 1 --slave /usr/local/bin/pip2 pip2 "${pip2}" || (echo "set python2 alternatives failed" && exit 1)
        ...
        update-alternatives --install /usr/local/bin/python3 python3 "${py3}" 1 --slave /usr/local/bin/pip3 pip3 "${pip3}" || (echo "set python3 alternatives failed" && exit 1)
        update-alternatives --install /usr/local/bin/python3 python3 "${py_custom}" 2 --slave /usr/local/bin/pip3 pip3 "${pip_custom}" || (echo "set python3 alternatives failed" && exit 1)
        update-alternatives --auto python3
        update-alternatives --display python3

        update-alternatives --remove-all pip || true
        update-alternatives --remove-all python || true
        update-alternatives --install /usr/local/bin/python python /usr/local/bin/python2 1 --slave /usr/local/bin/pip pip /usr/local/bin/pip2 || (echo "set python alternatives failed" && exit 1)
        update-alternatives --install /usr/local/bin/python python /usr/local/bin/python3 2 --slave /usr/local/bin/pip pip /usr/local/bin/pip3 || (echo "set python alternatives failed" && exit 1)
