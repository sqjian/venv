...
function install_cmake_from_source() {
    apt-get update -y
    apt-get install -y wget gzip curl git openssl libssl-dev coreutils build-essential clang lldb lld
...
function install_python_from_source() {
    build_python_from_source() {
        apt-get update -y
        apt-get install -y \
            make build-essential libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev wget curl llvm libncurses5-dev libncursesw5-dev xz-utils tk-dev liblzma-dev tk-dev coreutils
